...
class LogicalClientRuntime:
    ...
    def __init__(
        ...
        candidates: Mapping[str, SplitCandidateDescriptor],
        *,
        ...
        *,
        ...
        def execute() -> None:
            ...
            if "server_forward_ms" not in server_phase or "server_backward_ms" not in server_phase:
                # Missing phases remain missing; they must never be fabricated
                # as zero-valued component observations.
                raise RuntimeError(
                    "The split runtime reported no server forward/backward phase split "
                    f"(measured keys: {sorted(server_phase)}); CoSplit-UCB requires the "
            ...
            holder.update(
                ...
                client_backward_ms=backward_ms,
                server_forward_ms=float(server_phase["server_forward_ms"]),
                server_backward_ms=float(server_phase["server_backward_ms"]),
