...
class BackendAdapter(Protocol):
    ...
    def tied_state_groups(self, model: Any) -> tuple[tuple[int, ...], ...]:
        """Group state positions that are one shared tensor under several names.

        Positions index :meth:`state_manifest` order. A tied weight is written
        by whichever stage uses it, so both the prefix and the suffix can
        legitimately update the same logical parameter.
        """
        ...
