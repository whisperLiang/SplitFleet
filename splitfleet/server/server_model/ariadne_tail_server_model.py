"""Ariadne suffix server model for client-prefix split learning."""
...
def _load_model_from_ndarrays(model: torch.nn.Module, ndarrays: list[np.ndarray]) -> None:
    ...
    if len(state_dict) != len(ndarrays):
        raise ValueError(
            "Ariadne tail server-model parameter mismatch: "
...
class AriadneTailServerModel(ServerModel):
    """Execute the Ariadne suffix for split-learning rounds."""
    ...
    def __init__(
        ...
        *,
        ...
        self.optimizer = None
        self.runtime_handle: AriadneRuntimeHandle | None = None
        self.num_examples = 0
        self.loss_total = 0.0
        self.sid = ""
    ...
    def _require_runtime_handle(self) -> AriadneRuntimeHandle:
        if self.runtime_handle is None:
            raise RuntimeError("AriadneTailServerModel has not been configured.")
        return self.runtime_handle


AutoSplitTailServerModel = AriadneTailServerModel
